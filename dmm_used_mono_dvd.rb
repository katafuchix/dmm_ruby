...
urls = []
urls.push("http://www.dmm.co.jp/mono/dvd/-/list/=/list_type=dmp/")
...
for num in 1..1667 do
  url = "http://www.dmm.co.jp/mono/dvd/-/list/=/list_type=dmp/page=" + num.to_s + "/"
  #puts url
  urls.push(url)
end
...
Anemone.crawl(urls, opts) do |anemone|
  anemone.on_every_page do |page|
    ...
    items.each do |item|
		...
		b = ItemsUsedDvdMono.exists?(:link => url)
		if b then
			next
		end
		...
		begin
			...
			# price
			price = item.xpath("div[1]/div[@class=\"value\"]/p[@class=\"price\"][1]").text
			puts price
			
			used_price = item.xpath("div[1]/div[@class=\"value\"]/p[@class=\"price\"][2]").text
			puts used_price
			...
			@item = ItemsUsedDvdMono.new(:site=>site, :link => url, :title=>title,
															:package=>package, :price=>price, :used_price=>used_price,
															:off=>off, :description=>description,
															:actress=>actress, :release_date=>release_date, 
															:rdate=>rdate)
			begin
				ItemsUsedDvdMono.transaction do
					@item.save!
				end
			rescue
			end
