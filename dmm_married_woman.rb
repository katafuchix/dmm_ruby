...
urls = []
urls.push("http://www.dmm.co.jp/digital/videoa/-/list/=/article=keyword/id=1039/sort=ranking/")
...
#for num in s...e do
#  url = "http://www.dmm.co.jp/digital/videoa/-/list/=/article=keyword/id=1039/sort=ranking/page=" + num.to_s + "/"
  #puts url
#  urls.push(url)
#end
  ...
  anemone.on_every_page do |page|
    ...
    items.each do |item|
		...
		begin
			...
			#puts rdate
			@item = ItemsMarriedWoman.new(:site=>site, :link => url, :title=>title,
															:package=>package, :price=>price, :description=>description,
															:actress=>actress, :rdate=>rdate)
			begin
				ItemsMarriedWoman.transaction do
					@item.save!
				end
			rescue
			end
