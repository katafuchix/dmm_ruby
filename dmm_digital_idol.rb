...
urls = []
urls.push("http://www.dmm.com/digital/idol/-/list/=/sort=review_rank/")
urls.push("http://www.dmm.com/digital/idol/-/list/=/sort=date/")
...
#for num in 1...400 do
#  url = "http://www.dmm.com/digital/idol/-/list/=/sort=review_rank/page=" + num.to_s + "/"
  #puts url
#  urls.push(url)
#end
...
Anemone.crawl(urls, opts) do |anemone|
  anemone.on_every_page do |page|
    ...
    items.each do |item|
		#puts item
		site = "http://www.dmm.com/digital/idol/"
		url = ""
		...
		b = ItemsDigitalIdol.exists?(:link => url)
		puts b
		if b then
			next
		end
		...
		begin
			...
			@item = ItemsDigitalIdol.new(:site=>site, :link => url, :title=>title,
															:package=>package, :price=>price, :description=>description,
															:rate=>rate,
															:actress=>actress, :rdate=>rdate)
			begin
				ItemsDigitalIdol.transaction do
					@item.save!
				end
			rescue
			end
