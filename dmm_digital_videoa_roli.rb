...
require "yaml"
require "mysql2"
require './ItemsDigitalVideoaRoli.rb' 
...
urls = []
urls.push("http://www.dmm.co.jp/digital/videoa/-/list/=/price_max=300/sort=ranking/trans_type=st/")

for num in 1..10 do
  url = "http://www.dmm.co.jp/digital/videoa/-/list/=/price_max=300/sort=ranking/trans_type=st/page=" + num.to_s + "/"
  #puts url
  urls.push(url)
end
...
Anemone.crawl(urls, opts) do |anemone|
  anemone.on_every_page do |page|
    ...
    items.each do |item|
		...
		b = ItemsDigitalVideoa300.exists?(:link => url)
		if b then
			next
		end
